      ...
      <div className="flex-shrink-0 border-b border-gray-700 bg-gray-800 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <h2 className="text-lg font-semibold text-white">子模块管理</h2>
            <span className="px-2 py-0.5 bg-purple-600 text-white text-xs rounded-full">{submodules.length} 个</span>
          </div>
          <div className="flex items-center space-x-2">
            <button onClick={fetchSubmodules} disabled={actionLoading} className="p-2 hover:bg-gray-700 rounded-md text-gray-400 hover:text-white">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {submodules.length > 0 ? (
          <div className="space-y-3">
            {submodules.map((sm) => (
              <div key={sm.path} className="bg-gray-800 rounded-lg border border-gray-700 p-4 hover:border-gray-600">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    <div className="p-2 bg-purple-600/20 rounded-lg"><FolderGit2 className="w-5 h-5 text-purple-400" /></div>
                    <div>
                      <h4 className="text-white font-medium">{sm.name || sm.path}</h4>
                      <p className="text-sm text-gray-400 mt-1">{sm.path}</p>
                      <div className="flex items-center space-x-3 mt-2 text-xs text-gray-500">
                        <code className="px-1.5 py-0.5 bg-gray-700 rounded">{sm.commit?.substring(0, 7) || '-'}</code>
                  ...
                  <div className="flex items-center space-x-1">
                    <button onClick={() => handleAction('/api/submodules/update', 'POST', { path: sm.path })} disabled={actionLoading}
                      className="p-1.5 hover:bg-gray-700 rounded text-gray-400 hover:text-green-400" title="更新">
                    ...
                    <button onClick={() => { if(confirm(`删除 ${sm.path}?`)) handleAction(`/api/submodules/${encodeURIComponent(sm.path)}`, 'DELETE') }} disabled={actionLoading}
                      className="p-1.5 hover:bg-gray-700 rounded text-gray-400 hover:text-red-400" title="删除">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md bg-gray-800 rounded-xl shadow-2xl border border-gray-700">
            <div className="px-6 py-4 border-b border-gray-700 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white">添加子模块</h3>
              <button onClick={() => setShowAdd(false)} className="text-gray-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm text-gray-400 mb-1">仓库 URL</label>
                <input type="text" value={newSubmodule.url} onChange={(e) => setNewSubmodule({...newSubmodule, url: e.target.value})}
                  placeholder="https://github.com/user/repo.git" className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white" />
              </div>
              <div>
                <label className="block text-sm text-gray-400 mb-1">本地路径</label>
                <input type="text" value={newSubmodule.path} onChange={(e) => setNewSubmodule({...newSubmodule, path: e.target.value})}
                  placeholder="libs/my-lib" className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white" />
              </div>
              <div>
                <label className="block text-sm text-gray-400 mb-1">分支（可选）</label>
                <input type="text" value={newSubmodule.branch} onChange={(e) => setNewSubmodule({...newSubmodule, branch: e.target.value})}
                  placeholder="main" className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white" />
              </div>
